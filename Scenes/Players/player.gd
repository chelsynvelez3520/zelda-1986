...
func _physics_process(delta: float) -> void:
	...
	#hacia la direccion donde este mirando y luego, pausamos 
	#la animacion
...
func move_screen_transition_player(direction: Vector2, distance: float) -> void:
	can_move = false
	# verificar la direccion del personaje y hacia esa direccion
	# voy a ejecutar la animacion
	
	if direction.x >0:
		animated_sprite_2d.play("walk_rigth")
	elif direction.x <0:
		animated_sprite_2d.play("walk_left")
	elif direction.y >0:
		animated_sprite_2d.play("walk_down")
